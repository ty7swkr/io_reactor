...
#ifndef IO_REACTOR_REACTOR_REACTOR_H_
#define IO_REACTOR_REACTOR_REACTOR_H_
...
{
public:
  using IoDemuxer = IoHandleDemuxer<EventHandler *, nullptr>;

  Reactor();
  bool  init(int32_t max_clients, size_t  max_events,
...
private:
  enum
  {
    EVENT_TIMEOUT_ADD = IoDemuxer::EVENT_USER,
    EVENT_TIMEOUT_DEL,
    EVENT_STOP
  };

  bool dispatch                     (const std::vector<IoDemuxer::EventData> &events);
  void dispatch_reactor_event       (const IoDemuxer::EventData &event, bool &is_stop_event);
  void dispatch_demuxer_event_io    (const IoDemuxer::EventData &event);
  void dispatch_demuxer_event_result(const IoDemuxer::EventData &event);
...
Reactor::register_event_handler(EventHandler *handler, const io_handle_t &io_handle)
{
  ...
  return demuxer_.register_read_event(io_handle, handler, true);
}
...
Reactor::remove_event_handler(EventHandler *handler)
{
  ...
  return demuxer_.remove_all_events(handler->io_handle_, handler, true);
}
...
Reactor::register_writable(EventHandler *handler)
{
  ...
  return demuxer_.register_write_event(handler->io_handle_,
                                       handler,
                                       false);
}
                     ...
                     const uint32_t &msec)
{
  ...
  return demuxer_.raise_user_event(EVENT_TIMEOUT_ADD,
                                   handler->io_handle_,
...
Reactor::unset_timeout(EventHandler *handler)
{
  ...
  return demuxer_.raise_user_event(EVENT_TIMEOUT_DEL,
                                   handler->io_handle_);
}
...
inline bool
Reactor::dispatch(const std::vector<IoDemuxer::EventData> &events)
{
  bool is_stop_event = false;
  for (const IoDemuxer::EventData &event : events)
  {
...
Reactor::run_timeout_handler()
{
  std::deque<std::unordered_set<EventHandler *>> objects =
      timer_.extract_timeout_objects();
...
Reactor::run_reactor_handler()
{
  ...
  {
    reactor_handler_->reactor_handle_error(errno,
