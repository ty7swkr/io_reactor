...
#ifndef IO_REACTOR_TCP_REACTOR_TCPSESSIONHANDLER_H_
#define IO_REACTOR_TCP_REACTOR_TCPSESSIONHANDLER_H_
...
using TCPSessionHandlerPtr = std::shared_ptr<TCPSessionHandler>;

class TCPSessionHandler
{
public:
  TCPSessionHandler(const sockaddr_storage &client_addr)
  : addr_(client_addr)
  { this->set_socket_address(client_addr); }

  virtual ~TCPSessionHandler() { event_handler_.reset(); }
...
protected:
  ...
  /**
   * TCPSessionHandler::set_timeout(ms)에 지정한 시간이 되면 호출됨.
   */
   ...
   */
  virtual void handle_error     (const int &error_no, const std::string &error_str) = 0;
  virtual void handle_sent_error(const int &error_no, const std::string &error_str,
...
public:
  ...
  const io_handle_t &io_handle()    const;
...
private:
  std::shared_ptr<TCPEventHandler> event_handler_;
  friend class TCPEventHandler;
};

}

#endif /* IO_REACTOR_TCP_REACTOR_TCPSESSIONHANDLER_H_ */
