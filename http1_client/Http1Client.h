...
#ifndef IO_REACTOR_HTTPS_CLIENT_HTTP1CLIENT_H_
#define IO_REACTOR_HTTPS_CLIENT_HTTP1CLIENT_H_
...
class Http1Client : protected TcpAsyncClient
{
public:
  Http1Client(Reactor &reactor, const size_t &recv_buffer_size = 10240)
  : TcpAsyncClient(reactor, recv_buffer_size), websocket_(false) {}
  virtual ~Http1Client() {}
...
protected:
  ...
  bool          unset_timeout         (const uint64_t &stream_id);

protected:
  bool          is_websocket          () const { return websocket_.load(); }
  static bool   is_switching_websocket(const Http1Response &reponse);
...
private:
  ...
  void          handle_sent_error     (const int      &err_no,  const std::string &err_str, const int32_t  &stream_id,
                                       const uint8_t  *data,    const size_t      &size)    override;
...
inline bool
Http1Client::set_timeout(const uint32_t &msec, const uint64_t &stream_id)
{
  int32_t min_msec = timer_.register_timeout(msec, stream_id);

  if (min_msec < 0)
    return false;

  TcpAsyncClient::set_timeout(min_msec);
  return true;
}

inline bool
Http1Client::unset_timeout(const uint64_t &stream_id)
{
  return timer_.remove_timeout(stream_id);
}

inline void
Http1Client::handle_timeout()
{
  int32_t min_msec = 0;
...
inline bool
Http1Client::is_switching_websocket(const Http1Response &response)
{
  if (response.status() != 101)
    return false;

  if (response.header.contains("sec-webSocket-accept") == false)
    return false;
...
inline bool
Http1Client::send(const Http1Request &request)
{
  if (websocket_.load() == true) return false;

  std::lock_guard<std::mutex> guard(requests_h1_lock_);
  requests_h1_.insert(std::make_pair(request.stream_id, request));
  return TcpAsyncClient::send(request.stream_id, request.packet());
}

inline bool
Http1Client::send(const int32_t &stream_id, const WebSocket &request)
{
  if (websocket_.load() == false) return false;

  std::lock_guard<std::mutex> guard(requests_ws_lock_);
  requests_ws_.insert(std::make_pair(stream_id, request));
  return TcpAsyncClient::send(stream_id, request.packet().data(), request.packet().size());
}

inline bool
Http1Client::connect(const std::string  &ip,
                     const uint16_t     &port,
                     const int32_t      &timeout_msec)
{
  ...
  return TcpAsyncClient::connect(ip, port, timeout_msec);
}

inline void
Http1Client::handle_recv(const std::vector<uint8_t> &data)
{
...
inline void
Http1Client::handle_recv_http1(const std::vector<uint8_t> &data)
{
...
inline void
Http1Client::handle_recv_ws(const std::vector<uint8_t> &data)
{
...
inline void
Http1Client::handle_sent(const int32_t &stream_id, const uint8_t *data, const size_t &size)
{
  (void)data; (void)size;
  if (websocket_.load() == false)
  {
    ...
    {
      std::lock_guard<std::mutex> guard(requests_h1_lock_);
      auto it = requests_h1_.find(stream_id);
  ...
  {
    std::lock_guard<std::mutex> guard(requests_ws_lock_);
    auto it = requests_ws_.find(stream_id);
...
inline void
Http1Client::handle_sent_error(const int &err_no, const std::string &err_str,
...
inline void
Http1Client::handle_error(const int &err_no, const std::string &err_str)
{
  if (websocket_.load() == false)
  {
...
#endif /* IO_REACTOR_HTTPS_CLIENT_HTTP1CLIENT_H_ */
