...
protected:
  ...
  {
    reactor_trace << acceptor_.addr_port();
    reactor_trace << io_handle_ << reactors_.handler_count() << reactor_->handler_count();
//    reactor_->set_timeout(this, 3000);
  }
...
int main(void)
{
  ...
  Acceptor acceptor;
  acceptor.listen_uds("./mysock.sock");

  ClientHandlerFactory factory(acceptor, reactors);
